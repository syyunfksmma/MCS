...
- Visual Studio + workloads installed (logs stored in `artifacts/setup/vs_install_20250929.txt`).
- Node 20.11 & pnpm 9.x detected (`pnpm --version`).
- SQL Express instance reachable (`sqlcmd -Q "SELECT @@VERSION"`).
- Repository dependency restore successful.
