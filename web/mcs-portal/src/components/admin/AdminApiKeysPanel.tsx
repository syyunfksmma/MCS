...
const scopeOptions: {
...
}[] = [
  {
    label: 'Workspace',
    value: 'workspace',
    description: 'Workspace 전용 읽기/쓰기 권한'
  },
...
export default function AdminApiKeysPanel({
            ...
            <Tooltip title="키 복사">
              <Button
                ...
                onClick={() => {
                  ...
                  message.success(
                    '마스킹된 키를 복사했습니다. 실제 키는 발급 시에만 확인 가능합니다.'
                  );
                }}
              />
            </Tooltip>
  ...
  const handleIssue = async () => {
    ...
    const expiresAt = new Date(
      Date.now() + values.expiresIn * 24 * 60 * 60 * 1000
    ).toISOString();
      ...
      Modal.confirm({
        ...
        content: (
          <Space direction="vertical">
            ...
            <Text type="secondary">
              확인을 누르면 새 API 키가 생성되며, 이번 화면에서만 전체 키가
              노출됩니다.
            </Text>
          </Space>
        ),
  ...
  const handleRevoke = async () => {
    ...
    if (confirmation !== 'DELETE') {
      revokeForm.setFields([
        { name: 'confirmation', errors: ['DELETE 를 정확히 입력하세요.'] }
      ]);
      return;
    }
    setLoading(true);
        ...
        prev.map((key) =>
          key.id === revokeModal.key.id
            ? {
                ...key,
                status: 'revoked',
                maskedKey: `${key.maskedKey.slice(0, 9)}✱✱✱${key.maskedKey.slice(-4)}`
              }
            : key
        )
      );
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <Form form={issueForm} layout="vertical" preserve={false}>
          ...
          <Form.Item
            label="스코프"
            name="scope"
            rules={[{ required: true, message: '스코프를 선택하세요.' }]}
          >
            <AccessibleSelect
              id="issue-api-scope"
              ...
              { required: true, message: '만료 일수를 입력하세요.' },
              {
                validator: (_, value) => {
                  if (typeof value !== 'number')
                    return Promise.reject(new Error('숫자를 입력하세요.'));
                  if (value < 7)
                    return Promise.reject(
                      new Error('최소 7일 이상이어야 합니다.')
                    );
                  if (value > 180)
                    return Promise.reject(
                      new Error('최대 180일까지 설정 가능합니다.')
                    );
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <Paragraph>
            <Text strong>{revokeModal?.key.label}</Text> 키를 영구 폐기합니다.
            복구가 불가능하며, 애플리케이션에서 즉시 사용이 중단됩니다.
          </Paragraph>
          <Form form={revokeForm} layout="vertical" preserve={false}>
            <Form.Item
              label="폐기 사유"
              name="reason"
              rules={[{ required: true, message: '사유를 작성하세요.' }]}
            >
              <Input.TextArea
                rows={3}
                placeholder="예: 테스트 키 만료, 보안 사고 대응"
                maxLength={200}
                showCount
              />
            </Form.Item>
      ...
      >
        <Paragraph>
          <Text type="secondary">
            발급된 키는 이번 화면에서만 전체 값이 표시됩니다. 반드시 보관하세요.
          </Text>
        </Paragraph>
        <Paragraph>
