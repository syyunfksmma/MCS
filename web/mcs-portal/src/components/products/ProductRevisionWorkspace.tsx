...
import type { ExplorerItem, ExplorerRevision, ExplorerRouting, ExplorerSource } from "@/types/explorer";
...
export default function ProductRevisionWorkspace({ product, generatedAt, source, searchParams }: ProductRevisionWorkspaceProps) {
  const router = useRouter();
  const pathname = usePathname();
  const query = useSearchParams();
  const revisions = product.revisions;
  const [selectedRevisionId, setSelectedRevisionId] = useState<string>(() => {
    const fromQuery = query.get("rev") || (typeof searchParams.rev === "string" ? searchParams.rev : undefined);
    if (!fromQuery) {
  ...
  // Ensure query-selected revision falls back gracefully when dataset changes server-side.
    return revisions.find((rev) => rev.id === selectedRevisionId) ?? revisions[0];
  ...
  const columns = useMemo(() => (selectedRevision ? createColumns(selectedRevision) : []), [selectedRevision]);

  if (!selectedRevision) {
    return (
      <Empty description="No revisions available" className="my-10" />
    );
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-col">
            ...
            <Text className="text-slate-200">
              Workspace generated {new Date(generatedAt).toLocaleString()} · Source: {source}
            </Text>
          </div>
          <Space size="large" align="center">
            <div className="flex flex-col items-end">
              <Text className="text-xs uppercase tracking-wide text-sky-100">Active Revision</Text>
              <Select
                value={selectedRevision.id}
                onChange={setSelectedRevisionId}
                options={revisions.map((rev) => ({ label: rev.code, value: rev.id }))}
                size="large"
              />
            </div>
            ...
            <Card
              ...
              title={
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <Text className="font-semibold text-slate-700">{column.title}</Text>
                    {column.description ? (
                      <Text className="text-xs text-slate-500">{column.description}</Text>
                    ) : null}
                  </div>
                  <Tag color="blue">{column.routings.length} Routings</Tag>
                </div>
              }
              extra={<Button size="small" disabled>Add Routing</Button>}
              className="min-h-[18rem] shadow-sm"
            >
              {column.routings.length === 0 ? (
                <Empty description="No routing assigned" image={Empty.PRESENTED_IMAGE_SIMPLE} />
              ) : (
                <Space direction="vertical" size="middle" className="w-full">
                  {column.routings.map((routing) => (
                    <div key={routing.id} className="rounded border border-slate-200 bg-white p-3 shadow-sm">
                      <header className="flex items-center justify-between">
                        <div>
                          <Text className="text-sm font-semibold">{routing.code}</Text>
                          <div className="text-xs text-slate-500">CAM Rev {routing.camRevision}</div>
                        </div>
                        <Tag color={STATUS_COLORS[routing.status]}>{routing.status}</Tag>
                      </header>
                      <ul className="mt-2 flex flex-wrap gap-2 text-xs text-slate-600">
                        {routing.files.map((file) => (
                          <li key={file.id} className="rounded bg-slate-100 px-2 py-1">
                            {file.name}
                          </li>
                        ))}
                      </ul>
